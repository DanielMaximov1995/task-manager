...
const SettingsComp = (props: OrgSettingsType) => {
    ...
    const isAdmin = organization.admin?.find(emailMember => emailMember === data?.user?.email)
    ...
    const handleLeaveOrganization = () => {
        const updateOrg = {
            ...organization,
            members : organization.members.filter(member => member.toLowerCase() !== data?.user?.email?.toLowerCase()),
            admin : organization.members.filter(member => member.toLowerCase() !== data?.user?.email?.toLowerCase())
        }
